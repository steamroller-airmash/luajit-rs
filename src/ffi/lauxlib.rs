...
extern "C" {
  ...
  pub fn luaL_loadfilex(L: *mut lua_State, filename: *const c_schar, mode: *const c_schar)
    -> c_int;
...
#[inline(always)]
pub unsafe fn luaL_typename(L: *mut lua_State, i: c_int) -> *const c_schar {
...
pub unsafe fn lua_ref(L: *mut lua_State, lock: c_int) -> c_int {
  if lock != 0 {
    ...
    lua_pushstring(
      L,
      b"unlocked references are obsolete\x00".as_ptr() as *const c_schar,
    );
    lua_error(L);
    0
  }
