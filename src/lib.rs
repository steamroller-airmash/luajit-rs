...
pub use lua::*;
pub use state::{OwnedState, State};
pub use types::{LuaFunction, LuaObject};

pub use libc::c_int;

/// This macro is used to wrap a rust function in an `extern "C"` trampoline
/// to automatically pass a [`State`](state/struct.State.html) struct as the
/// first argument instead of a `lua_State` raw pointer
///
/// # Examples
///
/// ```
...
///
/// use luajit::{OwnedState, State, c_int, ThreadStatus};
///
/// fn return_42(state: &mut State) -> c_int {
///     state.push(42);
///
...
///     assert_eq!(status, Ok(ThreadStatus::Normal));
///     
///     // Equivalent
